    ...
    <v-container fluid>
        <v-row>
            ...
            <v-col 
                ...
                md="2"
            >
                <v-btn block @click="checkCompanies" color="primary">
                    Поиск
                </v-btn>
            </v-col>
        </v-row>
        ...
        <v-card>
                            ...
                            <v-container class="pa-1">
                                <v-item-group>
                                    <v-row>
                                        ...
                                        >
                                            <v-item>
                                                <v-checkbox v-model="tempSettings" :label="header.text" :value="header.value">
                                                </v-checkbox>
                                            </v-item>
                                        </v-col>
                                    </v-row>
            ...
            >
                                ...
                                <th :class="header.class" :key="header.value">
                                    ...
                                    <v-tooltip v-if="header.sortable" transition="none" bottom>
                                        <template v-slot:activator="{ on, attrs }">
                                            ...
                                            >
                                                <template>
                                                    <v-icon :color="getColor(header)" v-if="header.sort_desc">mdi-sort-alphabetical-descending</v-icon>
                                                    <v-icon :color="getColor(header)" v-else>mdi-sort-alphabetical-ascending</v-icon>
                                                </template>
                                            </v-btn>
                                        </template>
                ...
                <template v-slot:item.TITLE="{ item }">
                    ...
                    >
                        <a :href="`https://${domainInfo.domain}/crm/company/details/${item.ID}/`" target="_blank">{{ item.TITLE }}</a>
                     </v-input>
                </template>
            </v-data-table>
        </v-card>
...
export default {
    ...
    computed: {
        filteredHeaders() {
            return this.headers.filter(header => header.displayed);
        },
        sortedHeaders() {
            return [...this.headers].filter(header => header.sort_order > -1)
    ...
    created() {
        ...
        this.columnOptions = JSON.parse(this.companyFields)
                                 .filter(option => !headerTitles.includes(option.title));
    ...
    methods: {
        ...
        acceptSettings() {
            ...
            this.headers.forEach(header => {
                ...
                if(!header.displayed) {
                    ...
                    if (header.value == this.groupBy[0]) {
                        this.groupBy = null;
                    }
                }
            });
        ...
        addColumn(local) {
            let optionIndex = this.columnOptions.findIndex(option => option.title == this.selectedHeader);
        ...
        onGroupBy(arg) {
            if(arg && arg.length > 0) {
                let header = this.headers.find(header => header.value == arg[0]);
